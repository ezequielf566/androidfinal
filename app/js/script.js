...
if(norm==='#ffffff' || norm==='#fff' || norm==='white' || /rgb\s255\s*,\s255\s,\s255\s/i.test(s)){
...
if(p==='#ffffff' || p==='white' || /rgb\s255\s*,\s255\s,\s255\s/i.test(s)){
el.setAttribute('stroke', '#000000');
}
}catch(){ if(/^#fff(?:fff)?$/i.test(s)) el.setAttribute('stroke','#000000'); }
}
});
}catch(){}
...
padX = Math.max(0, Math.round((TARGET_WSS - drawW)/2));
padY = Math.max(0, Math.round((TARGET_HSS - drawH)/2));
}
